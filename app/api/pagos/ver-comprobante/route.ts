...
export async function GET(request: Request) {
  ...
  if (excedeLimite(`ver-comprobante:${decoded.uid}`, 30)) {
    return NextResponse.json(RESPUESTA_LIMITE, { status: 429 });
  }

  const path = new URL(request.url).searchParams.get("path");
  if (!path || !path.startsWith("payment-receipts/")) {
  ...
  const { data, error } = await supabaseAdmin.storage.from(DOCS_BUCKET).createSignedUrl(path, 120);
  if (error || !data) {
