...
export async function listTemplates(): Promise<ClassTemplate[]> {
  const snap = await getDocs(
    query(collection(db, "classTemplates"), orderBy("diaSemana"), orderBy("hora"))
  );
  return snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<ClassTemplate, "id">) }));
}
...
/**
 * Genera las `classSessions` de los próximos `dias` a partir de las
 * `classTemplates` activas. Reemplaza a la Cloud Function programada
 * (que requeriría el plan Blaze): la admin la corre a mano cuando quiere
 * poblar la semana. Id determinístico (`${templateId}_${fecha}`), así que
...
export async function generarSesionesDesdePlantillas(dias = 7): Promise<number> {
  ...
  const hoy = new Date();
  const fechas = Array.from({ length: dias }, (_, i) => addDays(hoy, i));
...
export function subscribeToWeekSessions(
  ...
  return onSnapshot(q, (snap) => {
    cb(snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<ClassSession, "id">) })));
  });
}
...
export async function reservarCupo(sessionId: string, uid: string) {
...
export async function listBookingsForSession(sessionId: string): Promise<BookingConAlumno[]> {
  ...
  return bookings.map((booking, i) => ({
    booking,
    alumno: alumnos[i] ? { uid: alumnos[i]!.uid, nombre: alumnos[i]!.nombre, foto: alumnos[i]!.foto } : null,
  }));
}
